...
def get_quantile_limit(data, quantile=0.95, min_limit=1e-3):
    """根据分位数计算对称范围，用于处理长尾分布。"""
    if data is None or len(data) == 0:
        return min_limit
    q_val = np.nanquantile(np.abs(data), quantile)
    if not np.isfinite(q_val) or q_val == 0:
        return min_limit
    return q_val

def plot_comparison_with_error_dist(all_dft_energy, all_mlp_energy, all_dft_forces, all_mlp_forces, output="val-dp-test.png", energy_q=1.0, force_q=0.9):
    """绘制能量和力的对比图及其误差分布"""
    ...
    max_err = get_quantile_limit(data, quantile=energy_q, min_limit=0.01)
    ...
    data = force_errors
    max_err_f = get_quantile_limit(data, quantile=force_q, min_limit=0.05)
    weights = np.ones_like(data) * (100.0 / len(data))
...
def run(result_prefix, output="val-dp-test.png", energy_q=1.0, force_q=0.99):
    """
    ...
    4. Specify quantiles for error distribution plots:
       python3 dp-test.py --energy_q=0.99 --force_q=0.95

    :param result_prefix: Glob pattern for the result prefix (e.g., '**/000' or '30-dp-test/test').
    :param output: Output plot filename.
    :param energy_q: Quantile for energy error distribution axis (default 1.0).
    :param force_q: Quantile for force error distribution axis (default 0.9).
    """
    ...
    plot_comparison_with_error_dist(all_dft_energy, all_mlp_energy, all_dft_forces, all_mlp_forces, output=output, energy_q=energy_q, force_q=force_q)
