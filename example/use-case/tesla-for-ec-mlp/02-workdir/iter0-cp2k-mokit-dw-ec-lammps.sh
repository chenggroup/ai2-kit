...
#type_map=[Cu,H,O,Cl]
#sel_type=[2,3]
#sys_charge_map=[0,1,6,7]
#model_charge_map=[-8,-8]
...
[ -f $SCREENING_DIR/screening.done ] && echo "skip screening" || {
    for SYS_NAME in $SYS_NAMES; do
        ...
        ai2-kit tool model_devi \
            ...
            grade --lo 0.20 --hi 0.60 --col max_devi_f - \
   ...
   #        ai2-kit tool model_devi \
   #            read "$i" --traj_file dump.lammpstrj --md_file model_devi.out --specorder "[Cu,Cl,H,O,K,F]" --ignore_error - \
   #            grade --lo 0.05 --hi 0.35 --col max_devi_f - \
