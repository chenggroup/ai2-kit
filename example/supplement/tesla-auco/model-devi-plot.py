...
def parse_tsv(file_path: str, drop_pattern: Optional[str] = None):
    """
    Parse the tsv file.
    Aggregate values from all entries except the SUMMARY row.
    drop_pattern: regex string to exclude specific files based on the 'file' column.
    """
    try:
        with open(file_path, 'r') as f:
            ...
            data_rows = []
            regex = re.compile(drop_pattern) if drop_pattern else None
            ...
            for line in lines[1:]:
                ...
                # Apply drop logic
                if regex and regex.search(parts[idx_file]):
                    continue
...
def run(*files, out_file="./model-devi-stats.png", drop: Optional[str] = None):
    """
    CLI tool to plot model-devi stats.
    files: glob patterns for tsv files.
    drop: regex string to exclude specific rows based on their file name.
    """
    all_files = []
    ...
    results = []
    for f in all_files:
        res = parse_tsv(f, drop_pattern=drop)
        if res:
            results.append(res)
