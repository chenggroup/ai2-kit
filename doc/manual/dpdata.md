...
| ---------- | --------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------- |
| read       | Read dataset into memory. This command by itself is useless, you should chain other command after reading data into memory. | `ai2-kit tool dpdata read ./path/to/dataset --fmt deepmd/npy`                                  | Support [path pattern](./tips.md#path-pattern), can be call multiple time |
...
# instead of spelling out the depth of the dataset with wildcard, you can search the whole
# workdir for the file that identifies a dataset, and then read its parent directory,
# see `path pattern` in tips for the details of the `/./` notation
ai2-kit tool dpdata read './workdir/**/type.raw/./..' --fmt deepmd/npy - write ./merged_dataset

# the same trick works for any format, for example to read every VASP calculation in workdir
ai2-kit tool dpdata read './workdir/**/OUTCAR' --fmt vasp/outcar - write ./merged_dataset
