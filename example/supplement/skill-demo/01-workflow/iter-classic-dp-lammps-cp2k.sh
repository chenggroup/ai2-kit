...
[ $UPDATE_MD_CONFS -gt 0 ] && {
    # replace the initial structures for MD with the good structures to accelerate the sampling efficiency
    rm -f $WORK_DIR/lammps-data/* || true
    ai2-kit tool ase read $SCREENING_DIR/good.xyz -\
