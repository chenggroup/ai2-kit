...
check_env ITER_NAME
check_env Labeling_num
check_env TRAINING_STEPS
check_env EXPLORE_STEPS
check_env FREQ
check_env UPDATE_INITIAL_STR
...
if [ -f "$LABELING_DIR/cp2k-setup.done" ]; then
    echo "$SYS_NAME: skip cp2k setup"
else
    echo "🧩 Filtering non-empty decent.xyz files ..."
    FILES=$(find "$PREV_ITER_DIR/screening" -name "decent.xyz" -type f ! -empty | sort)


    if [ -z "$FILES" ]; then
        echo "⚠️ No non-empty decent.xyz files found under $PREV_ITER_DIR/screening/"
        exit 1
    fi

    echo "📦 Concatenating all xyz files ..."
    mkdir -p "$LABELING_DIR/tmp"
    ALL_XYZ="$LABELING_DIR/tmp/all_decent.xyz"

    # 合并所有非空的xyz到一个文件中
    cat $FILES > "$ALL_XYZ"

    echo "🎯 Sampling $Labeling_num structures from merged xyz ..."
    ai2-kit tool ase read "$ALL_XYZ" - sample "$Labeling_num" - \
    ...
    omb combo \
        ...
        make_files $LABELING_DIR/job-{i:03d}/cp2k_diag.inp --template $CONFIG_DIR/cp2k/cp2k_diag.inp - \
...
[ -f $DW_DIR/dw-setup.done ] && echo "skip dw setup" || {
    ...
    ai2-kit tool dpdata read $ITER_DIR/cp2k/job-*/ --fmt cp2k/dplr --cp2k_output="output" --wannier_file="wannier.xyz" --type_map="[Cu,H,O]" --sel_type="[2]" --model_charge_map="[-8]" - write $ITER_DIR/new-dataset
...
[ -f $LMP_DIR/setup.done ] && echo "$SYS_NAME: skip lammps setup" || {
    for SYS_NAME in $SYS_NAMES; do
        ...
        if [ "$UPDATE_INITIAL_STR" == "TRUE" ]; then
            ai2-kit tool ase read $PREV_ITER_DIR/screening/$SYS_NAME/update.xyz - write_dplr_lammps_data $EX_DATA_DIR/{i:03d}.data --type_map [Cu,H,O] --sel_type [2] --sys_charge_map [0,1,6] --model_charge_map [-8]
        else
    ...
    for SYS_NAME in $SYS_NAMES; do
        ...
        for i in "$LMP_DIR"/task/"$SYS_NAME"/job-330K*/; do
            ...
            ai2-kit tool model_devi \
                ...
                grade --lo 0.10 --hi 0.25 --col max_devi_f - \
...
# 3. CRITICAL: Safely remove 'X' atoms from update.xyz and recalculate atom counts for each frame
        if [ -s "$SCREENING_TASK_DIR/update.xyz" ]; then
            echo "Cleaning 'X' atoms and updating headers in update.xyz..."
            awk '
            {
                if (expected == 0) {
                    # Start of a new frame. Print previous frame if exists.
                    if (out != "") { printf "%d\n%s\n%s", count, header, out }
                    expected = $1     # Original atom count
                    count = 0         # New atom count (excluding X)
                    out = ""          # Coordinate buffer
                    read_lines = 0    # Track comment line
                } else if (read_lines == 0) {
                    header = $0       # Save comment line
                    read_lines = 1
                } else {
                    if ($1 != "X") {  # Only keep non-X atoms
                        count++
                        out = out $0 "\n"
                    }
                    expected--        # Countdown original atoms
                }
            }
            END {
                # Print the last frame
                if (out != "") { printf "%d\n%s\n%s", count, header, out }
            }' "$SCREENING_TASK_DIR/update.xyz" > "$SCREENING_TASK_DIR/update_clean.xyz"
            
            # Replace the original file with the cleaned one
            mv "$SCREENING_TASK_DIR/update_clean.xyz" "$SCREENING_TASK_DIR/update.xyz"
        fi
        
        # 4. Clean up empty files to prevent downstream errors
