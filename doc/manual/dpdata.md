...
| ---------- | --------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------- |
| read       | Read dataset into memory. This command by itself is useless, you should chain other command after reading data into memory. | `ai2-kit tool dpdata read ./path/to/dataset --fmt deepmd/npy`                                  | Support wildcard, can be call multiple time. Use `--recursive` to walk a directory recursively and read every dataset in it, the file name to search for is inferred from `--fmt` and can be overridden by `--file_name` |
...
# instead of spelling out the depth of the dataset with wildcard, you can walk the directory
# recursively with --recursive, the file name to search for is inferred from --fmt
# (for example type.raw for deepmd/npy, OUTCAR for vasp/outcar)
ai2-kit tool dpdata read ./workdir --recursive --fmt deepmd/npy - write ./merged_dataset

# use --file_name to override the inferred file name, wildcard is supported
ai2-kit tool dpdata read ./workdir --recursive --file_name "*.OUTCAR" --fmt vasp/outcar - write ./merged_dataset
